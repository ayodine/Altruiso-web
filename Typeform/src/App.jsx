...
import { submitFormResponse } from './firebase';
import './App.css';
...
function ShortTextInput({ value, inputType = 'text', prefix, suffix, required, submitMode, submitting, submitError, onChangeValue, onDone }) {
  const ref = useRef(null);
  useEffect(() => {
    const timer = setTimeout(() => {
      if (ref.current) ref.current.focus({ preventScroll: true });
  ...
  const canProceed = !required || (value != null && String(value).trim() !== '');
  const placeholders = { email: 'name@company.com', url: 'https://…', number: '0', tel: '+44 …' };

  return (
    <div className="text-input-wrapper">
      <div className={'short-input-row' + (prefix ? ' has-prefix' : '') + (suffix ? ' has-suffix' : '')}>
        ...
        <input
          ...
          placeholder={placeholders[inputType] || 'Type your answer here…'}
          value={value ?? ''}
          onChange={e => onChangeValue(e.target.value)}
          onKeyDown={e => {
            if (e.key === 'Enter') { e.preventDefault(); if (canProceed && !submitting) onDone(); }
          }}
          disabled={submitting}
        />
      ...
      {submitMode ? (
        <>
          ...
          <button className="submit-button" onClick={onDone} disabled={submitting || !canProceed}>
            {submitting ? (
        ...
        canProceed && (
          <div className="ok-button-container">
            <button className="ok-button" onClick={() => canProceed && onDone()}>OK <Check size={20} /></button>
            <span className="press-enter">press <strong>Enter ↵</strong></span>
          </div>
        )
      )}
...
function ContactInput({ fields, value, onChangeValue, onDone }) {
  const ref = useRef(null);
  useEffect(() => {
    const timer = setTimeout(() => {
      if (ref.current) ref.current.focus({ preventScroll: true });
