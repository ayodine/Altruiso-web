...
function createProgram(gl, shaderSources, tfVaryings, attribLocations) {
  ...
  [gl.VERTEX_SHADER, gl.FRAGMENT_SHADER].forEach((type, ndx) => {
    const s = createShader(gl, type, shaderSources[ndx]);
...
class InfiniteGridMenu {
  ...
  #initTexture() {
    ...
    Promise.all(this.items.map((item, i) => new Promise(resolve => {
      const x = (i % this.atlasSize) * cellSize;
      const y = Math.floor(i / this.atlasSize) * cellSize;
      
      if (item.iconSvg) {
        // Draw the dark background for the icon circle
        ctx.fillStyle = '#101215';
        ctx.fillRect(x, y, cellSize, cellSize);
        
        // Render a subtle circular border around the icon to give it depth
        ctx.beginPath();
        ctx.arc(x + cellSize/2, y + cellSize/2, cellSize/2 - 16, 0, Math.PI * 2);
        ctx.strokeStyle = 'rgba(2, 118, 232, 0.15)';
        ctx.lineWidth = 4;
        ctx.stroke();
        
        // Load and draw SVG path
        const img = new Image();
        img.onload = () => {
          const iconSize = 220; // Scale of the icon inside the 512px canvas cell
        ...
        // Fallback to loading standard image
        const img = new Image();
  ...
  #findNearestVertexIndex() {
    ...
    const nt = vec3.transformQuat(vec3.create(), n, invOrient);
...
const defaultItems = [{ image: 'https://picsum.photos/900/900?grayscale', link: '#', title: '', description: '' }];
...
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // No WebGL2 context available (e.g. context limit reached). Fail
      // gracefully instead of throwing and crashing the whole page.
      console.warn('InfiniteMenu: WebGL2 unavailable, skipping render.', err);
      return;
    }
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {activeItem && (
        <>
          {activeItem.tag && (
            <span className={`face-tag ${isMoving ? 'inactive' : 'active'}`}>
              {activeItem.tag}
            </span>
          )}
